...
- `frontend/getwrite-config` is a tracked symlink to the repo-root `getwrite-config/` (same pattern as `.claude/skills/shadcn`), letting static project-type-template imports resolve at a consistent relative depth from both the real tree and the shadow build root. **Flag for follow-up:** this symlink is untested on Windows checkouts/CI runners — worth a note or a cross-platform contributor callout if Windows becomes a supported dev environment.
