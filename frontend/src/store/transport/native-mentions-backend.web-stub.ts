/**
 * @module store/transport/native-mentions-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-resource-excerpts-backend.web-stub.ts`. Turbopack cannot
 * statically prove that `lib/api/mentions.ts`'s
 * `await import("../../store/transport/native-mentions-backend")` branch is
 ...
 * the literal specifier `../../store/transport/native-mentions-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-mentions-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared mentions core, none of which Turbopack's
 * client/SSR chunking context supports — hence the substitution.
 *
 * This stub is never actually invoked: `resolveMentionsTransport()` only
 ...
 *
 * Tests and `tsc` resolve the *real* `native-mentions-backend.ts` directly
 ...
 */
export function createNativeMentionsTransport(): MentionsTransport {
  throw new Error(
    "native-mentions-backend.web-stub: the native mentions transport was " +
