...
/**
 * @module store/transport/native-project-actions-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-revision-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `project-actions-controller.ts`'s
 * `await import("./transport/native-project-actions-backend")` branch is
 ...
 * the literal specifier `./transport/native-project-actions-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-project-actions-backend.ts` transitively imports `node:path` and
 * the storage layer via the shared project CRUD core, none of which
 * Turbopack's client/SSR chunking context supports — hence the
 * substitution.
 *
 * This stub is never actually invoked: `resolveProjectActionsTransport()`
 ...
 *
 * Tests and `tsc` resolve the *real* `native-project-actions-backend.ts`
 ...
 */
export function createNativeProjectActionsTransport(): ProjectActionsTransport {
  throw new Error(
    "native-project-actions-backend.web-stub: the native project-actions " +
