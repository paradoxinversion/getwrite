 ...
 * {@link StorageContext} installed once by `native-bootstrap.ts` (FR5) is
 * used instead, with no per-operation rebinding. `nativeFilesystem()`
 * remains a defensive fallback for the (unsupported) case where no
...
export function createNativeQueryTransport(
  ...
  async function run<T>(fn: () => Promise<T>): Promise<T> {
    if (deps.fs) {
      ...
      // call only, matching how the search/revision spike tests exercise
