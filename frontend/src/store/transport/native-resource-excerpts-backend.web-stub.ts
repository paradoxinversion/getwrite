...
/**
 * @module store/transport/native-resource-excerpts-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-resource-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/resource-excerpts.ts`'s
 * `await import("../../store/transport/native-resource-excerpts-backend")`
 ...
 * `../../store/transport/native-resource-excerpts-backend` at this file for
 * `next build`/`next dev`). The real `native-resource-excerpts-backend.ts`
 * transitively imports `node:path` and the storage layer via the shared
 * excerpts core, none of which Turbopack's client/SSR chunking context
 * supports — hence the substitution.
 *
 * This stub is never actually invoked: `resolveResourceExcerptsTransport()`
 ...
 *
 * Tests and `tsc` resolve the *real* `native-resource-excerpts-backend.ts`
 ...
 */
export function createNativeResourceExcerptsTransport(): ResourceExcerptsTransport {
  throw new Error(
    "native-resource-excerpts-backend.web-stub: the native resource-" +
      "excerpts transport was reached in a web/desktop build. This stub " +
