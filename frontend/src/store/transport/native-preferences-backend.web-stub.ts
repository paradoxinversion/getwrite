...
/**
 * @module store/transport/native-preferences-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-project-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/preferences.ts`'s
 * `await import("../../store/transport/native-preferences-backend")` branch
 * is unreachable in the hosted/desktop build (the branch's guard is a
 ...
 * the literal specifier `../../store/transport/native-preferences-backend`
 * at this file for `next build`/`next dev`). The real
 * `native-preferences-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared project preferences core, none of which
 * Turbopack's client/SSR chunking context supports — hence the
 * substitution.
 *
 * This stub is never actually invoked: `resolvePreferencesTransport()` only
 * calls into the native branch when
 ...
 *
 * Tests and `tsc` resolve the *real* `native-preferences-backend.ts`
 ...
 */
export function createNativePreferencesTransport(): PreferencesTransport {
  throw new Error(
    "native-preferences-backend.web-stub: the native preferences transport " +
