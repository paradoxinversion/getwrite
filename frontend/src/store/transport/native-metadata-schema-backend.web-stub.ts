...
/**
 * @module store/transport/native-metadata-schema-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-query-backend.web-stub.ts`. Turbopack cannot statically prove that
 * `metadata-schema-transport-service.ts`'s
 * `await import("./transport/native-metadata-schema-backend")` branch is
 ...
 * the literal specifier `./transport/native-metadata-schema-backend` at this
 * file for `next build`/`next dev`). The real
 * `native-metadata-schema-backend.ts` transitively imports `node:path` and
 * the storage layer via the shared metadata-schema core, none of which
 * Turbopack's client/SSR chunking context supports — hence the substitution.
 *
 * This stub is never actually invoked: `resolveMetadataSchemaTransport()`
 ...
 *
 * Tests and `tsc` resolve the *real* `native-metadata-schema-backend.ts`
 ...
 */
export function createNativeMetadataSchemaTransport(): MetadataSchemaTransport {
  throw new Error(
    "native-metadata-schema-backend.web-stub: the native metadata-schema " +
