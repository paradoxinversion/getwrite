...
/**
 * @module store/transport/native-compile-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-project-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/compile.ts`'s
 * `await import("../../store/transport/native-compile-backend")` branch is
 ...
 * the literal specifier `../../store/transport/native-compile-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-compile-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared compile core, none of which Turbopack's
 * client/SSR chunking context supports — hence the substitution.
 *
 * This stub is never actually invoked: `resolveCompileTransport()` only
 ...
 *
 * Tests and `tsc` resolve the *real* `native-compile-backend.ts` directly
 ...
 */
export function createNativeCompileTransport(): CompileTransport {
  throw new Error(
    "native-compile-backend.web-stub: the native compile transport was " +
