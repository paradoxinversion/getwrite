...
/**
 * @module store/transport/native-project-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-revision-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/projects.ts`'s
 * `await import("../../store/transport/native-project-backend")` branch is
 ...
 * the literal specifier `../../store/transport/native-project-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-project-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared project CRUD core, none of which
 * Turbopack's client/SSR chunking context supports — hence the
 * substitution.
 *
 * This stub is never actually invoked: `resolveProjectsTransport()` only
 ...
 *
 * Tests and `tsc` resolve the *real* `native-project-backend.ts` directly
 ...
 * behavior.
 */
import type { ProjectsTransport } from "../../lib/api/projects";
 ...
 */
export function createNativeProjectsTransport(): ProjectsTransport {
  throw new Error(
    "native-project-backend.web-stub: the native projects transport was " +
