...
/**
 * @module store/transport/native-resource-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-project-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/resources.ts`'s
 * `await import("../../store/transport/native-resource-backend")` branch is
 ...
 * the literal specifier `../../store/transport/native-resource-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-resource-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared resource CRUD core, none of which
 * Turbopack's client/SSR chunking context supports — hence the
 * substitution.
 *
 * This stub is never actually invoked: `resolveResourcesTransport()` only
 ...
 *
 * Tests and `tsc` resolve the *real* `native-resource-backend.ts` directly
 ...
 * behavior.
 */
import type { ResourcesTransport } from "../../lib/api/resources";
 ...
 */
export function createNativeResourcesTransport(): ResourcesTransport {
  throw new Error(
    "native-resource-backend.web-stub: the native resources transport was " +
