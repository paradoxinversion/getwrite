...
/**
 * @module store/transport/native-export-backend.web-stub
 *
 * **ADR-021 seam — web-build substitute.** Mirrors
 * `native-project-backend.web-stub.ts`. Turbopack cannot statically prove
 * that `lib/api/export.ts`'s
 * `await import("../../store/transport/native-export-backend")` branch is
 ...
 * the literal specifier `../../store/transport/native-export-backend` at
 * this file for `next build`/`next dev`). The real
 * `native-export-backend.ts` transitively imports `node:path` and the
 * storage layer via the shared export core, none of which Turbopack's
 * client/SSR chunking context supports — hence the substitution.
 *
 * This stub is never actually invoked: `resolveExportTransport()` only
 ...
 *
 * Tests and `tsc` resolve the *real* `native-export-backend.ts` directly
 ...
 */
export function createNativeExportTransport(): ExportTransport {
  throw new Error(
    "native-export-backend.web-stub: the native export transport was " +
