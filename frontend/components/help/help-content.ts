  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "heading", content: ["Metadata source folders"] },
      {
        type: "paragraph",
        content: [
          "Any folder can be marked as a ",
          strong("metadata source"),
          ". Documents inside it can then be linked to other documents from the Metadata Panel — which is how the built-in project types offer ",
          strong("Characters"),
          ", ",
          strong("Locations"),
          ", and ",
          strong("Items"),
          " in the novel and serial types, and folders like ",
          strong("Research"),
          " or ",
          strong("Fact Checking"),
          " in others. The flag lives in the project type's template, so which folders are metadata sources is fixed when the project is created — it cannot be turned on or off per folder afterwards. Nothing depends on the folder's name, though: no folder is protected, none is pinned to a fixed position, and renaming one does not break its links.",
        ],
      },
    ],
  },
