...
// projects transport — `native-project-backend.ts` and everything
// transitively under it (`project-crud-core.ts` -> `project-loader.ts`/
// `project-creator.ts`/`io.ts`, which import `node:path`) must never enter
// the web/desktop `next build` output.
//
// Two things have to hold simultaneously for that exclusion to work, and this
// file checks both:
//
// 1. Nothing statically or dynamically imports `native-project-backend.ts`
//    except `lib/api/projects.ts`'s single dynamic `import()`.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-project-backend.ts",
);
const PROJECTS_TRANSPORT_SERVICE_RELATIVE_PATH = path.join(
  "src",
  "lib",
  "api",
  "projects.ts",
...
const WEB_STUB_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-project-backend.web-stub.ts",
);
...
describe("native-project-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/projects.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-project-backend(?!\.web-stub)["']/;
    ...
    for (const dir of dirsToScan) {
      ...
      for (const file of collectSourceFiles(dir)) {
        ...
        if (relative === PROJECTS_TRANSPORT_SERVICE_RELATIVE_PATH) continue;
  ...
  it("lib/api/projects.ts's native loader thunk carries the literal dynamic import specifier", () => {
    const contents = fs.readFileSync(
      path.join(frontendRoot, PROJECTS_TRANSPORT_SERVICE_RELATIVE_PATH),
      "utf8",
    );

    expect(contents).toContain(
      'import("../../store/transport/native-project-backend")',
    );
  });
  ...
  it("next.config.mjs aliases the native backend specifier to a node:*-free stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain("native-project-backend.web-stub");
  });
