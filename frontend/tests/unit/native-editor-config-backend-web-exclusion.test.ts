...
// editor-config transport — `native-editor-config-backend.ts` and everything
// transitively under it (`editor-config-core.ts` -> `io.ts`, which imports
...
// file checks both:
//
// 1. Nothing statically or dynamically imports `native-editor-config-backend.ts`
//    except `lib/api/editor-config.ts`'s single dynamic `import()`.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-editor-config-backend.ts",
);
const EDITOR_CONFIG_TRANSPORT_SERVICE_RELATIVE_PATH = path.join(
  "src",
  "lib",
  "api",
  "editor-config.ts",
...
const WEB_STUB_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-editor-config-backend.web-stub.ts",
);
...
describe("native-editor-config-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/editor-config.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-editor-config-backend(?!\.web-stub)["']/;
    ...
    for (const dir of dirsToScan) {
      ...
      for (const file of collectSourceFiles(dir)) {
        ...
        if (relative === EDITOR_CONFIG_TRANSPORT_SERVICE_RELATIVE_PATH)
          continue;
  ...
  it("lib/api/editor-config.ts's native loader thunk carries the literal dynamic import specifier", () => {
    const contents = fs.readFileSync(
      path.join(frontendRoot, EDITOR_CONFIG_TRANSPORT_SERVICE_RELATIVE_PATH),
      "utf8",
    );

    expect(contents).toContain(
      'import("../../store/transport/native-editor-config-backend")',
    );
  });
  ...
  it("next.config.mjs aliases the native backend specifier to a node:*-free stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain("native-editor-config-backend.web-stub");
  });
