// entity-highlighting Task 5: guards the same web-bundle exclusion
// established for other native backends
// (`native-resource-excerpts-backend-web-exclusion.test.ts`) for the
// entity-alias-table transport — `native-entity-alias-table-backend.ts` and
// everything transitively under it (`entity-alias-table.ts` ->
// `backlinks.ts`/`sidecar.ts`/`io.ts`, which import `node:path`) must never
...
//    `native-entity-alias-table-backend.ts` except
//    `lib/api/entity-alias-table.ts`'s single dynamic `import()`.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-entity-alias-table-backend.ts",
);
const ALIAS_TABLE_TRANSPORT_SERVICE_RELATIVE_PATH = path.join(
  "src",
  "lib",
  "api",
  "entity-alias-table.ts",
);
const WEB_STUB_RELATIVE_PATH = path.join(
  "src",
  "store",
  "transport",
  "native-entity-alias-table-backend.web-stub.ts",
);
...
describe("native-entity-alias-table-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/entity-alias-table.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-entity-alias-table-backend(?!\.web-stub)["']/;
    ...
    for (const dir of dirsToScan) {
      ...
      for (const file of collectSourceFiles(dir)) {
        ...
        if (relative === ALIAS_TABLE_TRANSPORT_SERVICE_RELATIVE_PATH) continue;
  ...
  it("lib/api/entity-alias-table.ts's native loader thunk carries the literal dynamic import specifier", () => {
    const contents = fs.readFileSync(
      path.join(frontendRoot, ALIAS_TABLE_TRANSPORT_SERVICE_RELATIVE_PATH),
      "utf8",
    );
  ...
  it("next.config.mjs aliases the native backend specifier to a node:*-free stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain(
      "native-entity-alias-table-backend.web-stub",
    );
  });
