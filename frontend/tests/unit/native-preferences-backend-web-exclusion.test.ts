...
// preferences transport — `native-preferences-backend.ts` and everything
// transitively under it (`project-preferences-core.ts` -> `io.ts`, which
// imports `node:path`) must never enter the web/desktop `next build`
// output.
//
// Two things have to hold simultaneously for that exclusion to work, and this
// file checks both:
//
// 1. Nothing statically or dynamically imports `native-preferences-backend.ts`
//    except `lib/api/preferences.ts`'s single dynamic `import()`.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-preferences-backend.ts",
);
const PREFERENCES_TRANSPORT_SERVICE_RELATIVE_PATH = path.join(
  "src",
  "lib",
  "api",
  "preferences.ts",
...
const WEB_STUB_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-preferences-backend.web-stub.ts",
);
...
describe("native-preferences-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/preferences.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-preferences-backend(?!\.web-stub)["']/;
    ...
    for (const dir of dirsToScan) {
      ...
      for (const file of collectSourceFiles(dir)) {
        ...
        if (relative === PREFERENCES_TRANSPORT_SERVICE_RELATIVE_PATH) continue;
  ...
  it("lib/api/preferences.ts's native loader thunk carries the literal dynamic import specifier", () => {
    const contents = fs.readFileSync(
      path.join(frontendRoot, PREFERENCES_TRANSPORT_SERVICE_RELATIVE_PATH),
      "utf8",
    );

    expect(contents).toContain(
      'import("../../store/transport/native-preferences-backend")',
    );
  });
  ...
  it("next.config.mjs aliases the native backend specifier to a node:*-free stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain("native-preferences-backend.web-stub");
  });
