...
// resources transport — `native-resource-backend.ts` and everything
// transitively under it (`resource-crud-core.ts` -> `revision.ts`/
// `trash.ts`/`io.ts`, which import `node:path`) must never enter the
// web/desktop `next build` output.
//
// Two things have to hold simultaneously for that exclusion to work, and this
// file checks both:
//
// 1. Nothing statically or dynamically imports `native-resource-backend.ts`
//    except `lib/api/resources.ts`'s single dynamic `import()`.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-resource-backend.ts",
);
const RESOURCES_TRANSPORT_SERVICE_RELATIVE_PATH = path.join(
  "src",
  "lib",
  "api",
  "resources.ts",
...
const WEB_STUB_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-resource-backend.web-stub.ts",
);
...
describe("native-resource-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/resources.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-resource-backend(?!\.web-stub)["']/;
    ...
    for (const dir of dirsToScan) {
      ...
      for (const file of collectSourceFiles(dir)) {
        ...
        if (relative === RESOURCES_TRANSPORT_SERVICE_RELATIVE_PATH) continue;
  ...
  it("lib/api/resources.ts's native loader thunk carries the literal dynamic import specifier", () => {
    const contents = fs.readFileSync(
      path.join(frontendRoot, RESOURCES_TRANSPORT_SERVICE_RELATIVE_PATH),
      "utf8",
    );

    expect(contents).toContain(
      'import("../../store/transport/native-resource-backend")',
    );
  });
  ...
  it("next.config.mjs aliases the native backend specifier to a node:*-free stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain("native-resource-backend.web-stub");
  });
