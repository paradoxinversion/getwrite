...
// file checks both:
//
// 1. Nothing statically or dynamically imports `native-project-types-backend.ts`
//    except `lib/api/project-types.ts`'s single dynamic `import()`.
// 2. `next.config.mjs` carries the `turbopack.resolveAlias` entry that
//    substitutes a stub for that exact import specifier at build time.
...
const NATIVE_BACKEND_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-project-types-backend.ts",
);
...
const WEB_STUB_RELATIVE_PATH = path.join(
  ...
  "store",
  "transport",
  "native-project-types-backend.web-stub.ts",
);
...
describe("native-project-types-backend web-bundle exclusion", () => {
  const frontendRoot = path.resolve(__dirname, "..", "..");

  it("is referenced (statically or dynamically) only from lib/api/project-types.ts", () => {
    const dirsToScan = ["src", "app"].map((d) => path.join(frontendRoot, d));
    const importRe = /native-project-types-backend(?!\.web-stub)["']/;
  ...
  it("lib/api/project-types.ts's native loader thunk carries the literal dynamic import specifier", () => {
    ...
    expect(contents).toContain(
      'import("../../store/transport/native-project-types-backend")',
    );
  });

  it("next.config.mjs aliases the native backend specifier to a stub for Turbopack builds", () => {
    ...
    expect(configContents).toContain("native-project-types-backend.web-stub");
  });
